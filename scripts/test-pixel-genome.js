...
const r = g.renderGenome(a, 24); assert.equal(r.length, 576); const m = g.mutateGenome(a, 'mutation'); assert.notDeepEqual(Array.from(m), Array.from(a));
...
const damaged = g.damage(r, 0.2, 'damage'); assert(damaged.flips > 0); assert(g.hammingBits(r, damaged.bits) === damaged.flips); assert.deepEqual(Array.from(g.renderGenome(a, 24)), Array.from(r));
console.log('pixel-genome behavioral tests passed');
